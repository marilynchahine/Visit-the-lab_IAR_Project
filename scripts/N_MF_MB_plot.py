...
def main_MF_MB(nb_tests):
    ...
    def get_q_table_MF(nb_tests, play_params):
        for i in range(nb_tests):
            ...
            np.save('all_data/data/MF_tables/agent_Q_MF_'+str(i)+'_.npy', agent.Q_MF)
    ...
    def get_MF_performance_with_q_tables(nb_tests, play_parameters):
        ...
        for i in range(nb_tests):
            ...
            nav_env = SocialGridworld(size=120)
            environment = Lab_env_HRI(nav_env, human)
            init_Q = np.load('all_data/data/MF_tables/agent_Q_MF_'+str(i)+'_.npy')
        ...
        np.save('all_data/data/MF_tables/reward_MF_on_MB.npy', rewards)
...
def main_MF_MB_plot(nb_tests):
    ...
    rewards = np.load('all_data/data/MF_tables/rewards_many.npy',
    ...
    MF_MB_rewards = np.load('all_data/data/MF_tables/reward_MF_on_MB.npy',
                            allow_pickle=True).item()
    ...
    np.save('all_data/data/MF_tables/all_rewards_MB_MF.npy', rewards)
    ...
    all_rewards = np.load('all_data/data/MF_tables/all_rewards_MB_MF.npy',
                        allow_pickle=True).item()
